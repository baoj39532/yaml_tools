...
- 提取结果导出为Excel（每行一个资源，列为Key）

### 4. 变量替换器
- 批量替换YAML中的变量并导出副本
- 变量格式为 `{{VAR}}`，允许空格变体（如 `{{ VAR }}`）
- 支持选择集群文件夹/命名空间文件夹/单个文件

### 5. Key提取器
- 从ConfigMap资源中提取所有去重的key
- 支持普通字符串key、YAML格式和Properties格式
- 支持YAML多文档（`---`分隔）
- 自动去重并按字母序排列
- 导出为Excel单列格式

### 6. 配置文件
- 比较器/提取器的Key配置与变量替换配置可保存到同一YAML文件
- 启动时可选择加载配置文件
...
3. 点击"提取信息"
4. 点击"导出提取结果到Excel"保存结果（每行一个资源、列为Key）

### 变量替换器

1. 选择集群文件夹、命名空间文件夹或单个YAML文件
2. 选择输出目录
3. 配置变量名和值（变量名不含双花括号，值可为空串）
4. 点击"执行替换并导出"，在输出目录生成替换后的副本

### Key提取器

1. 点击"选择文件夹"或"选择文件"
2. 点击"提取Key"
3. 查看提取结果（表格展示，按字母序排列）
4. 点击"导出到Excel"保存结果

**支持的格式**：
- 普通字符串key：直接提取key名称
- YAML格式：递归提取内部结构（支持多文档`---`分隔）
- Properties格式：解析properties文件格式
